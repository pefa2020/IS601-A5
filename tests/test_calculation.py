...
"""
Now we have to test NotImplemented
"""
...
def test_power_overflow():
    """
    Test that triggers the except block in calculate method by causing an OverflowError
    that leads to InvalidOperation when converting inf to Decimal.
    """
    with pytest.raises(OperationError, match="Calculation failed"):
        Calculation(operation="Power", operand1=Decimal("2"), operand2=Decimal("10000"))
